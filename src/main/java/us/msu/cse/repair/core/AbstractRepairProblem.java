...
public abstract class AbstractRepairProblem extends Problem {
    ...
    protected TestLineCoverageInterface proflTestCoverage;
    ...
    public AbstractRepairProblem(Map<String, Object> parameters) throws Exception {
        ...
        xiaMethod = (String) parameters.get("xiaMethod");
        if (xiaMethod != null) {
            proflMethodCoverage = new XiaMethodLineCoverage(this.xiaMethod);
        }

        xiaTest = (String) parameters.get("xiaTest");
        if (xiaTest != null) {
            proflTestCoverage = new XiaTestLineCoverage(this.xiaTest);
        }

        xiaCoverage = (String) parameters.get("xiaCoverage");
        if (xiaCoverage != null && proflMethodCoverage != null && proflTestCoverage != null) {
            profl = new ProflResultRanking(proflMethodCoverage, proflTestCoverage, this.xiaCoverage);
        }
...
//                
//                for(String s : positiveTests) {
//                    System.out.println(String.format("\t- Positive Test: %s", s));
//                }
        System.out.println("Number of negative tests: " + negativeTests.size());

        for (String s : negativeTests) {
            System.out.println(String.format("\t- Negative Test: %s", s));
        }
    ...
    void invokeTestFilter() throws IOException, InterruptedException {
        ...
        for (String s : positiveTests) {
            System.out.println(String.format("\t- Relevant Positive Test: %s", s));
        }
